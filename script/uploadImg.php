<?php
	...
	if (isset($_FILES["myfile"]) and $_POST["id"] <> "") {
		$output_dir = "../../assets/img/media/";
		$ret = array();
		
		//Création du dossier.
		mkdir("../../assets/img/media", 0755, true);
		...
		if (!is_array($_FILES["myfile"]["name"])) { //Single $_FILES
			...
			if (move_uploaded_file($_FILES["myfile"]["tmp_name"], $output_dir.$fileName)) {
				...
				$dataAddImg  = $bdd->prepare("INSERT INTO tbl_ent_imgs (EI_Src) VALUES (?)");
				$dataAddImg->execute([str_replace("../", "", $output_dir.$fileName)]);
